...
- renommer `total_tax` → `tax` et extraire `net`
- fractionner le `data.frame` (ligne trop longue)
- supprimer le `return()` explicite (retour implicite)
- ajouter la documentation **roxygen2**

```{.r}
#' Calcule l'impôt et le revenu net
#'
#' @param income Revenu brut (numérique ou vecteur).
#' @param rate Taux d'imposition, par défaut `0.2` (20 %).
#'
#' @return Un `data.frame` avec `income`, `tax`, `net` et `note`.
#' @export
#'
#' @examples
#' calculate_tax(c(1000, 2500))
calculate_tax <- function(income, rate = 0.2) {
  ...
  data.frame(
    ...
    tax    = tax,
    net    = net,
    note   = "Calculated automatically by custom financial routine"
  )
}
```
...
`tests/testthat/test-calculate-tax.R` :

```{.r}
test_that("calculate_tax calcule l'impôt et le net", {
  res <- calculate_tax(c(1000, 2500), rate = 0.2)

  expect_equal(res$tax, c(200, 500))
  expect_equal(res$net, c(800, 2000))
  expect_equal(res$income, c(1000, 2500))
})

test_that("calculate_tax applique le taux par défaut", {
  res <- calculate_tax(1000)

  expect_equal(res$tax, 200)
  expect_equal(res$net, 800)
})
```

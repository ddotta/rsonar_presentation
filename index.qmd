...
## Version 2 — Refactoriser et documenter

Améliorations manuelles :

- renommer `Part_SAU` → `calculer_part_sau` et `SAU_HA` → `sau_ha` (snake_case)
- supprimer les variables inutilisées (`unused_arg`, `var_inutile1`, `var_inutile2`)
- remplacer `SAU_HA == NA` → `is.na()` et `class() == "numeric"` → `is.numeric()`
- supprimer la boucle `for (i in 1:length(...)) print(i)` parasite
...
| 2 | Refactorisation + doc | 82 % | C | 0 % |
| 3 | Tests ajoutés | 96 % | A | 85 % |
...
La couverture n'est **mesurée** que lorsqu'il existe des tests. Avant (étapes 0 à 2), elle vaut 0 % et n'engendre **aucune pénalité**. À l'étape 3, les tests font passer la couverture **projet** à 85 % — au-dessus du seuil de 80 % — donc la pénalité `(80 − couverture) × 5 min` reste à **0**.

⚠️ Tester une seule fonction ne suffit pas : si la couverture projet restait faible (~10 %), la pénalité `(80 − 10) × 5 = 350 min` ferait retomber le score à **0 %**. C'est la couverture **globale** du projet qui compte.
:::
...
4. Voir la note monter : **38% → 62% → 82% → 96%**

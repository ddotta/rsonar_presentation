...
`sonar_analyse(".")` **sans** `include_coverage = FALSE` mesure la couverture dès qu'un dossier `tests/` existe. Ici, elle n'est que de **7 %** : `calculer_part_sau` est la seule fonction testée sur les 10 scripts.
...
Le package `rsonar` peut être intégré dans un pipeline CI/CD pour automatiser la mesure de la qualité et le reformatage du code.

- Voir [ce dépôt](https://gitlab.com/ddotta/rproject_for_rsonar)

`sonar_fix(".", create_mr = TRUE)` ouvre directement une Merge Request.
